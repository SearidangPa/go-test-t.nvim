...
function test_display:update_display_buffer(tests_info, pin_triggered)
    ...
    if
        not self.display_bufnr
        or not vim.api.nvim_buf_is_valid(self.display_bufnr)
    then
        return
    end
...
function test_display:create_window_and_buf()
    ...
    if
        not self.display_win_id
        or not vim.api.nvim_win_is_valid(self.display_win_id)
    then
        vim.cmd("vsplit")
...
function test_display:_parse_test_state_to_lines(tests_info)
    ...
    for _, test in ipairs(tests_table) do
        ...
        then
            ...
            table.insert(
                buf_lines,
                string.format("%s %s", status_icon, test.name)
            )
        end
    end
    return buf_lines
...
function test_display:_add_display_help_text(buf_lines)
    if
        self.display_win_id and vim.api.nvim_win_is_valid(self.display_win_id)
    then
...
function test_display:_setup_keymaps()
    ...
    map("n", "q", function()
        self_ref:_close_display()
    end, map_opts)
    map("n", "<CR>", function()
        self_ref:_jump_to_test_location_from_cursor()
    end, map_opts)
