...
function go_test:setup_user_command()
    local self_ref = self
    vim.api.nvim_create_user_command("TestBoard", function()
        self_ref.displayer:toggle_display()
    end, {})

    if self.integration_test_pkg and self.integration_test_pkg ~= "" then
        vim.api.nvim_create_user_command("TestIntegration", function()
            self_ref:test_pkg(self.integration_test_pkg)
        end, {})
    end
...
function go_test:_filter_golang_output(entry)
    ...
    if not test_info then
        vim.notify(
            "Filter Output: Test info not found for " .. entry.Test,
            vim.log.levels.WARN
        )
        return
    end
    ...
    local file, line_num_any =
        string.match(trimmed_output, "Error Trace:%s+([^:]+):(%d+)")
