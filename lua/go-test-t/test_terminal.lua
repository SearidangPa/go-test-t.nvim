...
function terminal_test:test_in_terminal(
    test_info,
    do_not_close_terminal,
    delete_terminal_after
)
    ...
    if do_not_close_terminal then
        vim.api.nvim_feedkeys(
            vim.api.nvim_replace_termcodes("G", true, false, true),
            "n",
            false
        )
        float_term_state =
    ...
    vim.schedule(function()
        vim.api.nvim_buf_attach(float_term_state.bufnr, false, {
            on_lines = function(_, buf, _, first_line, last_line)
                return self_ref:_process_buffer_lines(
                    buf,
                    first_line,
                    last_line,
                    test_info,
...
function terminal_test:test_nearest_in_terminal()
    ...
    if vim.bo.filetype == "lua" then
        ...
        test_command = string.format(
            [[nvim --headless --noplugin -u scripts/tests/minimal.vim -c "lua require('plenary.busted').run('%s', {filter='%s'})"]],
            rel_path,
            escaped_test_name
        )
    else
    ...
    require("go-test-t.util_lsp").action_from_test_name(
        ...
        function(lsp_param)
            ...
            local intermediate_path =
                util_path.get_intermediate_path(lsp_param.filepath)
...
function terminal_test:preview_terminal(terminal_name)
    local float_terminal_state =
        self.terminal_multiplexer.all_terminals[terminal_name]
    ...
    float_terminal_state.win =
        vim.api.nvim_open_win(float_terminal_state.bufnr, true, {
            relative = "editor",
            width = width,
            height = height - 3,
            row = 0,
...
function terminal_test._auto_update_test_line(_, test_info)
    local augroup = vim.api.nvim_create_augroup(
        "TestLineTracker_" .. test_info.name,
        { clear = true }
    )
...
---@param test_info terminal.testInfo
function terminal_test:_process_buffer_lines(
    buf,
    first_line,
    last_line,
    test_info,
...
function terminal_test:_process_one_line(line, test_info, current_time)
    ...
    if string.match(line, "--- FAIL") or string.match(line, "FATAL") then
        vim.notify(
            string.format("%s fail", test_info.name),
    ...
    elseif string.match(line, "--- PASS") then
        vim.notify(
            string.format("%s pass", test_info.name),
            vim.log.levels.INFO
        )
