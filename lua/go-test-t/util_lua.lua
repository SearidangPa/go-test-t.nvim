    ...
    if node:type() == 'function_call' then
      ...
      if func_name_node then
        ...
        if func_name == 'it' then
           ...
           if args_node then
             local first_arg = args_node:child(1) -- parentheses are children too usually, but let's check field
             ...
             for i = 0, args_node:named_child_count() - 1 do
               ...
               if child:type() == 'string' then
                 local test_name_node = child:child(1) -- string_content (start quote is child 0 in raw, but named child might be different)
