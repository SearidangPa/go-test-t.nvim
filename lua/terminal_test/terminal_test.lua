...
function terminal_test_M:process_one_line(line, test_info, float_term_state, current_time, cb_update_tracker)
  ...
  if string.match(line, '--- FAIL') then
    ...
    self:handle_test_failed(test_info, float_term_state, current_time, cb_update_tracker)
    return true
  ...
  elseif string.match(line, '--- PASS') then
    self:handle_test_passed(test_info, float_term_state, current_time, cb_update_tracker)
    return true
  end
end
...
function terminal_test_M:process_buffer_lines(buf, first_line, last_line, test_info, float_term_state, cb_update_tracker)
  ...
  for _, line in ipairs(lines) do
    local detach = self:process_one_line(line, test_info, float_term_state, current_time, cb_update_tracker)
    if detach then
  ...
  vim.schedule(function()
    vim.api.nvim_buf_attach(float_term_state.buf, false, {
      on_lines = function(_, buf, _, first_line, last_line)
        return self_ref:process_buffer_lines(buf, first_line, last_line, test_info, float_term_state, cb_update_tracker)
      end,
    })
  end)
end
